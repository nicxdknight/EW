 ...
 * @author harsh
 */
@WebServlet(name = "PlaceOrder", urlPatterns = {"/PlaceOrder", "/place_order.html"})
public class PlaceOrder extends HttpServlet {
    private final ItemDao itemData;

    public PlaceOrder() throws ClassNotFoundException, FileNotFoundException {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        order = itemData.cartToOrder();
         Double total=0d;
         total = itemData.getOrderTotal(order.getId());
          System.out.println(total);
          session.setAttribute("total", total);
          itemData.makeCartEmpty(email);
